export interface Asset {
  id: string;
  name: string;
  category: 'ruangan' | 'fasilitas';
  location: 'Gedung Agustinus' | 'Gedung Josephus' | 'Gedung Katarina';
  totalStock: number;
...
export interface DamageReport {
  ...
  status: 'menunggu' | 'dalam_perbaikan' | 'selesai';
  assignedTo?: string;
  createdAt: string;
  updatedAt: string;
}
...
export const getMockDashboardStats = (role: string): DashboardStats => {
  return {
